                ...
                <div class="container">
                    <div class="row">
                        <div class="col-sm-12 wow fadeIn">
                            ...
                            <h1>Tableau des intervenants /</h1>
                            <p><?php
                                $type_utilisateur=1;
                                $status=1;
                                $pdo = connect();
                                $req2 = $pdo->prepare('SELECT * FROM utilisateur WHERE status ='.$status.' AND type_utilisateur='.$type_utilisateur.'');
                                $req2->execute();   
                                $nbJobs = $req2->fetchALL(); 
                                echo count($nbJobs)." intervenants";
              ...
              <div class="form-section">
                <div class="row">
                    <form role="form">
                      ...
                      <input type="text" name="prenom" class="form-control input-sm" maxlength="64" placeholder="Prénom" />
                        </div>
                      </div>
                      <div class="col-md-3">
                        <div class="form-group">
                         <div class="input-group">
                             <input type="text" name="nom" class="form-control input-sm" maxlength="64" placeholder="Nom" /></div>
                        </div>
                      </div>
                                    ...
                                    <tbody>
                                            ...
                                            if (isset($_POST["prenom"])){
                                                ...
                                                if (!empty($_POST['prenom'])){
                                                    $sql1= ' AND prenom like "%'.$_POST['prenom'].'%"';
                                                }
                                                else{
                                                    $sql1='';
                                                }
                                                if (!empty($_POST['nom'])){
                                                    $sql2= ' AND nom like "%'.$_POST['nom'].'%"';
                                                }
                                                else{
                                                    $sql2='';
                                                }
                                                if (!empty($_POST['matieres'])){
                                                    $sql3= ' AND matieres like "%'.$_POST['matieres'].'%"';
                                                }
                                                else{
                                                    $sql3='';
                                                }
                                                if (!empty($_POST['ville'])){
                                                    $sql4= ' AND ville like "%'.$_POST['ville'].'%"';
                                                }
                                                else{
                                                    $sql4='';
                                                }
                                               
                                                $resultats=$pdo->query('SELECT * FROM utilisateur WHERE status ='.$status.' AND type_utilisateur='.$type_utilisateur.$sql1.$sql2.$sql3.$sql4.'');
                                                $resultats->setFetchMode(PDO::FETCH_OBJ);
                                            }
                                        ...
                                        <tr>
                                            <td>
                                                <img class="img-circle" src="<?php if($resultat->photo == null){ 
                                                    echo "/assets/img/photo/no_avatar.png"; 
                                                } else{
                                                    echo $resultat->photo; 
                                                    }
                                                ?>" alt="">
                                                <a href="http://alloJob974.fr/profil_Job2.php?id_utilisateur=<?php echo $resultat->id_utilisateur; ?>" class="user-link"><?php echo $resultat->prenom; echo " "; echo $resultat->nom; ?></a>
                                                <span class="user-subhead"><?php echo $resultat->matieres; ?></span>
                                            </td>
                                            <td><?php echo $resultat->telephone; ?></td>
                                            <td>
                                                <a href="mailto:<?php echo $resultat->email; ?>"><?php echo $resultat->email; ?></a>
                                            </td>
                                            <td>
