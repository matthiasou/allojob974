            ...
            <div class="container">
                <div class="row">
                    <div class="col-sm-12 wow fadeIn">
                        ...
                        <p>Ici vous pouvez voir vos prochaines interventions</p>
                    </div>
                </div>
            </div>
        </div>
...
$pdo = connect();
$accepte=1;
$notifications=$pdo->query('SELECT * FROM reponse_annonce WHERE id_utilisateur1 ='.$_SESSION['id_utilisateur'].' AND accepte='.$accepte.'');
$notifications->setFetchMode(PDO::FETCH_OBJ);
while( $resultat = $notifications->fetch() )
{

    $sql5 = "UPDATE reponse_annonce SET non_lu_prof = :non_lu_prof WHERE id_reponse_annonce = :id_reponse_annonce";
...
<div class="container">
    ...
    <thead>
        ...
        $resultats=$pdo->query('SELECT * FROM reponse_annonce WHERE id_utilisateur1 ='.$_SESSION['id_utilisateur'].' AND accepte='.$accepte.'');
        $resultats->setFetchMode(PDO::FETCH_OBJ);
        while( $resultat = $resultats->fetch() )
        {
        	$i++;
            $prenom = $pdo->query("SELECT prenom FROM `utilisateur` WHERE id_utilisateur='".$resultat->id_utilisateur."'");
        ...
        ?>
        <form id="<?php echo $i; ?>" method="POST" action="https://www.allojob974.fr/advert_details.php">
        	<input type="hidden" name="id_annonce2" value="<?php echo $Annonce2['id_annonce2']; ?>">
        <tr>
            <td class="text-center"><a href="advert_details.php?id_annonce2=<?php echo $Annonce2['id_annonce2']; ?>">Voir l'annonce</a></td>
