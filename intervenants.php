...
<!-- Top menu -->
  ...
  <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
    ...
    <li><a href="/faq">FAQ</a></li>
      ...
      ?>
        ...
        <!-- Slider 2 -->
                ...
                <div class="container">
                    <div class="row">
                        <div class="col-sm-12 wow fadeIn">
                            ...
                            <p>Voici les intervenants correspondant à vos critéres</p>
                        </div>
                    </div>
                </div>
            </div>   
            ...
            <div class="row-fluid col-xs-12 col-sm-8">
            <div class="">
            <?php
            $pdo = connect();
            $matiere = utf8_decode($_GET['matiere']);
            if (isset($_GET['matiere'])){
                $req2 = $pdo->prepare('SELECT count(*) FROM utilisateur WHERE matieres LIKE "%'.utf8_encode($matiere).'%" AND status=1');
                $req2->execute($filtre['argumentPDO']);   
                $nbJobs = $req2->fetchColumn(); 
            }
            ?>
                <h3 style="font-family: Lobster;"><?php if($nbJobs==0){echo"Aucun intervenant ne correspond à vos critéres";} elseif($nbJobs==1){echo "1 intervenant correspond à vos critéres";} else{ echo $nbJobs." intervenants correspondent à vos critéres";} ?></h3>
            </div>
            <br>
            <?php
            if (isset($_GET['matiere'])){
                $matiere = utf8_decode($_GET['matiere']);
                $req = $pdo->prepare('SELECT * FROM utilisateur WHERE matieres LIKE "%'.utf8_encode($matiere).'%" AND status=1 ORDER BY RAND()');
                ...
                <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xs-offset-0 col-sm-offset-0 col-md-offset-3 col-lg-offset-3 toppad" >
                 <div class="panel panel-info">
                    <div class="panel-heading"><h3 class="panel-title"><?php echo $donnees['prenom']; ?> (<?php $date = new DateTime($donnees['date_naissance']); $now = new DateTime(); $interval = $now->diff($date); echo $interval->y; ?> ans)</h3>
                    ...
                    <div class="panel-body">
                      <div class="row">
                        <div class="col-md-3 col-lg-3 " align="center">
                            ...
                            ?>" width="150px" height="250px" class="img-circle img-responsive">
                        </div>
                        <div class=" col-md-9 col-lg-9 "> 
                          <table class="table table-user-information"><tbody><tr><td><?php echo $donnees['situation']; ?> </td><td><?php echo $donnees['choix_precision']; ?> </td></tr><tr><td><?php $mots = explode(",", $donnees['matieres']); foreach ($mots as $mot) {
                              echo $mot; ?> &nbsp<?php
                          } ?></td><td> </td></tr></tbody></table>
                        </div>
                        <form method="POST" action="http://www.alloJob974.fr/profil.php">
                        <input type="hidden" name="id_utilisateur" value="<?php echo $donnees['id_utilisateur'] ?>">
                        <button type="submit" class="btn">Voir profil</button>
                        </form>
                      </div>
                    </div>
                  </div>
            ...
            <div class="row-fluid col-xs-12 col-sm-4">
                <div class="">
                    <h3 style="font-family: Lobster;">Devenez membre pour bénéficier de nos cours</h3>
                </div>
                    <br><br><br>
                    <?php
                    session_start();
    if (isset($_SESSION['nom']) && $_SESSION['type_utilisateur'] == 0){
        ?>
        <form action="add_annonces.php">
                        <button class="btn">Déposer une annonce</button> 
                    </form>

                    <?php 
    }
    elseif (isset($_SESSION['nom']) && $_SESSION['type_utilisateur'] == 1) {
        ?>
