...
<div class="form-group">
  ...
  <div class="col-md-4">
    <select id="situation" name="choixSituation" required="" onChange="remplirPrecision(this.options[this.selectedIndex].value);" class="form-control" value="<?php echo $_POST['choixSituation']; ?>">
      ...
      <option value="Un(e) débutant(e)">Un(e) débutant(e)</option>
      <option value="Un particulier expérimenté">Un particulier expérimenté</option>
...
function education_fields() {
  ...
  var rdiv = 'removeclass'+room;
    divtest.innerHTML = '<div class="form-group"><label class="col-md-4 control-label" for="ville">Service(s)</label><div class="col-md-4"><div class="input-group"><select class="form-control" required="" id="matiere" name="matiere[]" value="<?php echo $_POST['matiere[]']; ?>"><option value="">-- Choisir votre service --</option><option value="MENAGE">MENAGE</option><option value="DEMENAGEMENT">DÉMÉNAGEMENT</option><option value="GARDE D ENFANTS">GARDE D ENFANTS</option><option value="JARDINAGE">JARDINAGE</option><option value="REPASSAGE">REPASSAGE</option><option value="BRICOLAGE">BRICOLAGE</option><option value="COURS DIVERS">COURS DIVERS</option><option value="INFORMATIQUE">INFORMATIQUE</option><option value="PLOMBERIE">PLOMBERIE</option><option value="ELECTRICITE">ELECTRICITE</option><option value="PETITS TRAVAUX">PETITS TRAVAUX</option><option value="ADMINISTRATIF">ADMINISTRATIF</option><option value="GARDE D ANIMAUX">GARDE D ANIMAUX</option><option value="AUTRE">AUTRE</option> </select><div class="input-group-btn"> <button class="btn btn-danger" type="button" onclick="remove_education_fields('+ room +');"> <span class="glyphicon glyphicon-minus" aria-hidden="true"></span> </button></div><div class="clear"></div></div></div></div>';
...
<!-- Select Basic -->
  ...
  <div class="form-group">
  <label class="col-md-4 control-label" for="ville">Service(s)</label>
  <div class="col-md-4">
...
if (isset($_POST['nom']) && isset($_POST['prenom']) && isset($_POST['email']) && isset($_POST['adresse'])){
	if ($_POST['password'] == $_POST['password2']){
        ...
        if($results['id_utilisateur'] == null){
            ...
            $pedagogie=str_replace(array("\r\n","\n"),'<br>',$_POST['pedagogie']);
	        	...
	        	$req = $pdo->prepare("INSERT INTO utilisateur (nom, prenom, email, telephone, password, date_naissance, secu_sociale, ville_naissance, situation, adresse ,complement, code_postale, ville, vehicule, matieres, secteur, pedagogie, photo, status, type_utilisateur, civilite_id_civilite) VALUES (:nom, :prenom, :email, :telephone, :password, :date_naissance, :secu_sociale, :ville_naissance, :situation, :adresse ,:complement, :code_postale, :ville, :vehicule, :matieres, :secteur, :pedagogie, :photo, :status, :type_utilisateur, :civilite_id_civilite)");
	                $req->execute(array(
	                                    "nom" => $_POST['nom'],
	                                    "prenom" => $_POST['prenom'],
	                                    "email" => $_POST['email'],
	                                    ...
	                                    "situation" => $_POST['choixSituation'],
    ...
    /*          
        ...
        $template->setValue('choixSituation', $_POST['choixSituation']);
              ...
              $mail2->AddAddress('matthiasou@yahoo.fr'); 
	            ...
	            $mail2->SetFrom('contact@allojob974.fr', 'Allo Job 974');
              $mail2->AddReplyTo('contact@allojob974.fr', 'Allo Job 974'); 
	            ...
	            $mail->SetFrom('contact@allojob974.fr', 'Allo Job 974');
              $mail->AddReplyTo('contact@allojob974.fr', 'Allo Job 974');
              $mail->AddAttachment('Convention_ALLOJOB_974.docx');  
            ...
            $( document ).ready(function() {
                swal({
                   ...
                   text: "Votre profil va être analysé.",
                   type: "success",
            }, function(isConfirm) {
